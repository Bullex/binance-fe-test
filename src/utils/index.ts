...
export const getPrepairedData = (products: any, filters: any, search?: string, favoriteKeys?: ReactText[]) => {
  return Object.keys(products).map((key: string) => {
    ...
    if (filters) {
      switch (filters.tab) {
        ...
        case Enums.Common.ALTS: {
          const isPMPreset = item.parentMarket === Enums.Common.ALTS;
          let isPNPreset: boolean = false;

          if (filters.alts === Enums.Common.ALTS) {
            isPNPreset = true;
          } else {
        ...
        case Enums.Common.FIAT: {
          const isPMPreset = item.parentMarket === Enums.Common.FIAT;
          let isPNPreset: boolean = false;

          if (filters.fiat === Enums.Common.FIAT) {
            isPNPreset = true;
          } else {
