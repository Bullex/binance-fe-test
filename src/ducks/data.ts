...
const initialState = {
  ...
  filters: {
    ...
    alts: Enums.Common.ALTS,
    fiat: Enums.Common.FIAT
  },
  indicator: Enums.Indicator.CHANGE,
  search: '',
  favoriteKeys: []
}
...
export const saveIndicator = (indicator: string, dispatch: Dispatch) => {
  dispatch({ type: ActionTypes.SAVE_INDICATOR,
    payload: indicator
  })
}
...
const dataReducer = (state = initialState, action: Models.IAction) => {
  switch (action.type) {
    ...
    case ActionTypes.SAVE_INDICATOR:
